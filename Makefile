...
help:
	...
	@echo "  run27             install Context Chat for Nextcloud 27"
	@echo "  "
	...
	@echo "  register27 perform registration of running 'Context Chat' into the 'manual_install' deploy daemon."

# .PHONY: build-push
# build-push:
# 	docker login ghcr.io
# 	docker buildx build --push --platform linux/arm64/v8,linux/amd64 --tag ghcr.io/nextcloud/context_chat_backend:1.0.3 --tag ghcr.io/nextcloud/context_chat_backend:latest .
...
.PHONY: deploy27
deploy27:
	docker exec master-stable27-1 sudo -u www-data php occ app_api:app:unregister context_chat_backend --silent || true
	docker exec master-stable27-1 sudo -u www-data php occ app_api:app:deploy context_chat_backend docker_dev \
		--info-xml https://raw.githubusercontent.com/nextcloud/context_chat_backend/master/appinfo/info.xml

.PHONY: run27
run27:
	docker exec master-stable27-1 sudo -u www-data php occ app_api:app:unregister context_chat_backend --silent || true
	docker exec master-stable27-1 sudo -u www-data php occ app_api:app:register context_chat_backend docker_dev \
		--force-scopes \
		--info-xml https://raw.githubusercontent.com/nextcloud/context_chat_backend/master/appinfo/info.xml
...
register27:
	docker exec master-stable27-1 sudo -u www-data php occ app_api:app:unregister context_chat_backend --silent || true
	docker exec master-stable27-1 sudo -u www-data php occ app_api:app:register context_chat_backend manual_install --json-info \
  "{\"appid\":\"context_chat_backend\",\"name\":\"Context Chat Backend\",\"daemon_config_name\":\"manual_install\",\"version\":\"1.0.0\",\"secret\":\"12345\",\"host\":\"host.docker.internal\",\"port\":10034,\"scopes\":{\"required\":[],\"optional\":[]},\"protocol\":\"http\",\"system_app\":0}" \
  --force-scopes
