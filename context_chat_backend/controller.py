...
def enabled_handler(enabled: bool, nc: NextcloudApp | AsyncNextcloudApp) -> str:
	...
	MULTI_TASKTYPE_ID = 'context_chat:context_chat_multi'
	MULTI_PROVIDER_ID = 'context_chat-context_chat_multi'
		...
		if enabled:
			...
			multi_tasktype = TaskType(
				...
				output_shape=[
					ShapeDescriptor(
						name='questions',
						description='The individual questions that were asked, in order',
						shape_type=ShapeType.LIST_OF_TEXTS,
					),
					ShapeDescriptor(
						name='answers',
						description='The answers generated by the model, in the same order as the questions',
						shape_type=ShapeType.LIST_OF_TEXTS,
					),
					ShapeDescriptor(
						name='sources',
			...
			multi_provider = TaskProcessingProvider(
			...
			nc.providers.task_processing.unregister(MULTI_PROVIDER_ID)
